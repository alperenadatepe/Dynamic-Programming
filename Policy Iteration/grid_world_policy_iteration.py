...
class GridWorld():
    ...
    def choose_action_by_policy(self, hashed_state):
        action = np.random.choice(self.actions, p=self.Policy[hashed_state, ])
    ...
    def evaluate(self):
        while True:
            ...
            for state in self.states:
                ...
                action, action_index = self.choose_action_by_policy(hashed_state)
                next_state, reward = self.step(state, action)
                
                hashed_next_state = self.state_hasher(next_state)
                self.V[hashed_state] = self.state_transition[hashed_state, action_index] * (reward + self.discount_rate * self.V[hashed_next_state])
    ...
    def improve(self):
        ...
        for state in self.states:
            ...
            old_action, old_action_index = self.choose_action_by_policy(hashed_state)
...
if __name__ == "__main__":
    ...
    V, Policy = grid_world.policy_iteration()        
